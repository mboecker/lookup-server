get_nearest_neighbor = function(table_trafo_scaled, parameters_trafo_scaled, numeric_params) {
  ...
  non_numeric_params = setdiff(all_params, c(numeric_params))
  ...
  if (length(non_numeric_params)>0) {
    nnres = parameters_trafo_scaled[, c(get_nearest_neighbour_subset(.SD, mget(non_numeric_params))), by = non_numeric_params]
  } else {
