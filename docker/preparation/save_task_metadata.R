...
(SELECT data, quality, value FROM data_quality WHERE quality='NumberOfFeatures' OR quality='NumberOfInstances' OR quality = 'NumberOfClasses') t1
INNER JOIN
...
result = dplyr::rename(result, features = "NumberOfFeatures", instances = "NumberOfInstances", baseline = "NumberOfClasses")
result$baseline = 1 / type.convert(result$baseline)
