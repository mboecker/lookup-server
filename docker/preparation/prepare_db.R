...
replace_na_with_defaults = function(table, algo_id) {
  parameter_names = getParamIds(parameter_ranges[[algo_id]], TRUE, TRUE)
  parameter_names = intersect(parameter_names, colnames(table)) # should not be necessary, just for safety
  for(parameter_name in parameter_names) {
    nas = is.na(table[[parameter_name]])
    if(any(nas)) {
      def = get_parameter_default(algo_id, parameter_name)
      if (!is.null(def)) {
...
saveTableFile = function(task_id, algo_id) {
  ...
  if (file.exists(path)) {
    ...
    t = replace_na_with_defaults(t, algo_id)
